...
export const useUserProfile = (personId?: string) => {
  ...
  const updateEmergencyField = useCallback(async (fieldName: string, value: string) => {
    if (!personId) return false;
    ...
    try {
      ...
      if (result && typeof result === 'object' && 'success' in result && result.success) {
        toast.success('Emergency contact field updated successfully');
        await fetchProfile(personId);
        return true;
      } else {
        throw new Error((result as any)?.message || 'Update failed');
      ...
      toast.error(err.message || 'Failed to update emergency contact field');
      return false;
    }
  }, [personId]);
  ...
  const getCurrentEmergencyFields = useCallback(() => {
    if (!data?.emergencyContact) return [];

    const fields = [];
    
    if (data.emergencyContact.first_name) {
      fields.push({ key: 'first_name', label: 'First Name', value: data.emergencyContact.first_name, type: 'text' as const });
    }
    if (data.emergencyContact.last_name) {
      fields.push({ key: 'last_name', label: 'Last Name', value: data.emergencyContact.last_name, type: 'text' as const });
    }
    if (data.emergencyContact.email) {
      fields.push({ key: 'email', label: 'Email', value: data.emergencyContact.email, type: 'text' as const });
    }
    if (data.emergencyContact.phone_number) {
      fields.push({ key: 'phone_number', label: 'Phone Number', value: data.emergencyContact.phone_number, type: 'text' as const });
    }
    if (data.emergencyContact.relationship) {
      fields.push({ key: 'relationship', label: 'Relationship', value: data.emergencyContact.relationship, type: 'select' as const });
    }
