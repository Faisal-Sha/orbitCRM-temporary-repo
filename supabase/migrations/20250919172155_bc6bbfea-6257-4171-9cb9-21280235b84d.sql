...
BEGIN
  ...
  -- Get current role to check if transitioning from admin
  ...
  IF p_role_name = 'admin' AND current_role_name != 'admin' THEN
    ...
    IF agency_id IS NOT NULL THEN
      ...
      SELECT p_person_id, agency_id, current_user_id, current_user_id
      WHERE NOT EXISTS (
        SELECT 1 FROM public.app_agencies_admins 
        WHERE person_id = p_person_id AND agency_id = update_people_user_role.agency_id AND is_deleted = false
      );
    END IF;
