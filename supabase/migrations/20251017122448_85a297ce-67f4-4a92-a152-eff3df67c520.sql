-- Update sync_lead_to_mailerlite trigger to only sync CPST leads
...
BEGIN
  ...
  -- Check if the lead has CPST service in their latest form submission
  SELECT EXISTS (
    SELECT 1
    FROM public.forms_submissions fs
    WHERE fs.submitted_by_id = NEW.id
      AND fs.is_deleted = false
      AND (
        -- Check for CPST in various field name variations (case-insensitive)
        (fs.submission_data->>'service')::text ILIKE '%CPST%'
        OR (fs.submission_data->>'Service')::text ILIKE '%CPST%'
        OR (fs.submission_data->>'services')::text ILIKE '%CPST%'
        OR (fs.submission_data->>'Services')::text ILIKE '%CPST%'
      )
  ...
  IF NOT COALESCE(has_cpst_service, false) THEN
    RAISE LOG 'Lead % does not have CPST service - skipping MailerLite sync', NEW.id;
    RETURN NEW;
  END IF;
  ...
  VALUES (NEW.id, 'pending', 'Trigger fired for CPST lead, calling edge function');
  ...
  BEGIN
    ...
    SET sync_status = 'initiated', error_message = 'HTTP request sent for CPST lead with ID: ' || request_id
